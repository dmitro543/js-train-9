...
function findElementGreaterThan(arr, num) {
  ...
  const findelement = arr.find((el) => el > num);
...
function findIndexByProperty(arr, name) {
  ...
  const indexBy = arr.findIndex((obj => obj.name === name));
  return indexBy >=0 ? indexBy: "об'єкт не знайдено.";
}
...
function findLastEvenNumber(arr) {
  ...
  const findNumber = arr.findLast((el) => el % 2 === 0);
  return findNumber !== undefined
   ? findNumber
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const ConvertToString = arr.slice(startIdx).join(" ");
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const Filter = arr.flat();
  const Squarel = Filter.map((el) => Math.pow(el, 2));
  const Flatten = Squarel.filter((el) => el > threshold);
   return Flatten;
}
...
function wordLengths(arr) {
  ...
  const word = arr.map(function (element) {
...
function expandArrayByN(arr, n) {
  ...
  const result = arr.flatMap((el) => Array(n).fill(el));
  return result;
}
...
function findLongestWord(arr) {
  ...
  const findLongest = arr.reduce((acc, val) => {
...
function findDuplicateElements(arr) {
  ...
  const findDuplicate = arr.filter((item, index) => {
...
function capitalizeWords(arr) {
  ...
  const capitalize = arr.map(function (el) {
